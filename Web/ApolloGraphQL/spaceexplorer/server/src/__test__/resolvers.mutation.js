...
describe('[Mutation.bookTrips]', () => {
  ...
  const { getLaunchesByIds } = mockContext.dataSources.laucnhAPI;
  ...
  it('returns true if booking succeeds', async () => {
    ...
    // * Check if the dataSources was called with correct args
...
describe('[Mutation.cancelTrip]', () => {
  ...
  it('returns true if cancelling succeds', async () => {
    ...
    expect(cancelTrip).toBeCalledWith({ lauchId: 123 });
  });

  it('returns false if cancelling falis', async () => {
    cancelTrip.mockReturnValueOnce(false);

    // * cehck the resolver response
    const res = await resolvers.Mutation.cancelTrip(
      null,
      { launchId: 123 },
      mockContext
